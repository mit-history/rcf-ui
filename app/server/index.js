    ...
    match({ routes, location: req.url}, (error, redirectLocation, renderProps) => {
        if (error) {
            ...
            Promise.all(renderProps.components
                        ...
                        .map(c => c.fetchData({dispatch: store.dispatch,
                                               params: renderProps.params})))
                .then(() => {
                    const reactComp = ce(Provider, {store}, ce(RouterContext, renderProps));
                    const response = template
                              .replace('<!-- react-entry-point -->',
                                       renderToString(reactComp))
                              .replace('{/* react-initial-state */}',
