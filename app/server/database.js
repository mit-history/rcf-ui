...
const db = pgp({
    host: process.env.POSTGRES_HOST === undefined ? '/var/run/postgresql' :  process.env.POSTGRES_HOST,
...
function nbAuthors() {
  return db.query("SELECT COUNT(id) from person_agg").then(results => results[0].count);
}

function nbGenres() {
  return db.query("SELECT count(distinct n.normalized) from validated_plays AS p JOIN normalized_genres AS n ON (p.genre=n.genre)").then(results => results[0].count);
}

function nbPlays() {
  return db.query("SELECT COUNT(validated_plays.id) FROM validated_plays WHERE validated_plays.title != ''").then(results => results[0].count);
}

function nbSeasons() {
  return db.query("SELECT count(distinct season) from registers").then(results => results[0].count);
}
...
FROM registers r JOIN register_plays rp ON (r.id=rp.register_id)
                 JOIN play_person pp ON (rp.play_id=pp.play_id)
WHERE pp.person_id=$1 GROUP BY 1 ORDER BY 1
`, [id])
        .then(receipts => receipts.map(r => ({
...
FROM registers as r JOIN register_plays as rp ON (r.id=rp.register_id)
     ...
     JOIN play_person as pp ON (pp.play_id=p.id)
     JOIN normalized_genres AS n ON (p.genre=n.genre)
WHERE pp.person_id=$1 GROUP BY 1 ORDER BY 2 DESC
`, [id]);
}
...
FROM registers as r JOIN register_plays as rp ON (r.id=rp.register_id)
JOIN validated_plays as p ON (p.id=rp.play_id)
JOIN  play_person as pp ON (pp.play_id=p.id) JOIN normalized_genres AS n ON(p.genre=n.genre)
WHERE pp.person_id=$1 GROUP BY p.title, p.date_de_creation, p.genre, n.normalized, p.id ORDER BY p.title DESC

`, [id]).then(plays => plays.map(p => ({
    play_id: p.id,
    title: p.title,
    genre: p.genre,
...
function fetchAuthorInfos(id) {
    return db.query('SELECT * FROM person_agg where id=$1', [id])
        .then(records => records[0]);
}


export function fetchAuthor(id) {
    return fetchAuthorInfos(id)
        .then(authorData => {
            const queries = [
                ...
                mapPromiseToStore(authorData, authorPlaysOverview(id), 'playsOverview'),
                mapPromiseToStore(authorData, fetchLagrangeImages(id), 'lagrangeImages'),
            ];
...
export function fetchAuthorSerieDef(id) {
    return Promise.all([fetchAuthorInfos(id), authorReceipts(id)])
        .then(([infos, receipts]) => {
            return {
                name: infos.name,
...
FROM validated_plays AS p
  JOIN register_plays AS rp ON (p.id=rp.play_id)
  JOIN play_person AS pp ON (pp.play_id=p.id)
  JOIN person_agg AS pa ON (pp.person_id=pa.id)
  JOIN normalized_genres AS n ON (p.genre=n.genre)
WHERE p.id=$1
`, [id])
        .then(records => records[0])
...
FROM registers r JOIN register_plays rp ON (r.id=rp.register_id)
     ...
     JOIN play_person pp ON (p.id=pp.play_id)
     JOIN person_agg pa ON (pp.person_id=pa.id)
...
export function fetchSeasons() {
    return Promise.all([seasonInfos(), seasonMostPlayed(1), seasonMostPlayed(2)])
...
export function fetchSeason(id) {
    return Promise.all([seasonInfo(id), seasonPriceSeries(id)])
        .then(([{dates: registers, reprises, firsts}, priceSeries]) => ({registers, reprises, firsts, priceSeries}));
}
...
`, [season])
    .then(results => {
        ...
        const genreIdx = {};
        genres.forEach((g, i) => {genreIdx[g] = i});
        const matrix = [];
        names.forEach(() => {
        ...
        for (const date of Object.keys(byDate)) {
            ...
            matrix[genreIdx[genre2]][genreIdx[genre1] + genres.length + 1]++;
            matrix[genreIdx[genre1] + genres.length + 1][genreIdx[genre2]]++;
            total++;
        }
...
FROM registers as r
     ...
     JOIN play_person AS pp ON (pp.play_id=p.id)
     JOIN normalized_genres AS n ON (p.genre=n.genre)
WHERE pp.person_id=$1 AND r.season=$2
ORDER BY r.date`, [author, season]);
}
...
export function fetchRegister(date) {
    return Promise.all([registerInfos(date), prevnextDate(date)])
...
export function ftisearch(term) {
  return Promise.all([searchAuthors(term), searchPlays(term)])
        .then(([authors, plays]) => {
            return [
