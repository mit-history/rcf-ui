            ...
            authors.map(author =>
                ce(
                    ...
                    ce(
                        ...
                        ce(
                            Card,
                            { shadow: 2, style: { background: '#fff' } },
                            ce(CardTitle, {
                                expand: true,
                                style: {
                                    background: author.depict_urls[0]
                                        ? `url("${author
                                              .depict_urls[0]}") no-repeat center top`
                                        : `url(${buildURL(
                                              '/images/default-picto-author.png',
...
function authorPlaysOverview({ authorData }) {
    ...
    return ce(
        ...
        ce(
            CardText,
            { style: { margin: '0 auto', fontSize: '1em' } },
            ce(
                BootstrapTable,
                { data: authorData.playsOverview, hover: true },
                ce(
                    TableHeaderColumn,
                    {
                        dataField: 'title',
...
function authorGenreStatView({ authorData }) {
    const pieData = authorData.genres.map(g => ({
        label: g.genre,
        count: g.nbplayed,
        object: g,
    }));
    ...
    return ce(
        Card,
        { className: mdlclass({ col: 3, tablet: 12 }) },
        ...
        ce(
            ...
            ce(Pie, {
                ...
                urlformatter: evt =>
                    browserHistory.push(
                        buildURL(`/genre/${evt.point.object.genre}`),
                    ),
            }),
        ),
...
function authorMainCardView({ authorData }) {
    ...
    const properties = [
        ...
        {
            label: 'Recettes',
            value: `${numberWithSpaces(totalReceipt)} livres`,
        },
    ];
    ...
    return ce(
        ...
        ce(
            Card,
            { className: mdlclass({ col: 3, tablet: 6 }) },
            ce('img', {
                src:
                    authorData.depict_urls[0] ||
                        buildURL('/images/default-picto-author.png'),
...
function authorGalleryView({ authorData }) {
    const hasImages =
        (authorData.depict_urls.length && authorData.depict_urls[0]) ||
    ...
    return ce(
        Card,
        { className: classNames('mdl-shadow--2dp', mdlclass(12)) },
        ce(CardTitle, { style: STYLES.titleChart }, 'Images'),
        ce(
            CardText,
            { className: 'mdl-grid', style: { margin: 'auto' } },
            ...imageObjects.map(image =>
                ce(
                    'div',
                    {
                        className: classNames(
                            'mdl-shadow--2dp author-image-wrapper',
                            mdlclass({ col: 2, tablet: 4, phone: 2 }),
                        ),
                    ...
                    ce(
                        ...
                        ce(
                            'p',
                            { className: 'author-img-caption' },
                            ce(
                                'span',
                                { className: 'author-image-title' },
                                `${image.imgtitle}` !== 'undefined'
                                    ? `${image.imgtitle}`
                                    : '',
                            ),
                            ce(
                                'span',
                                { className: 'author-image-src' },
                                `${image.imgsrc}`,
                            ),
                        ),
                    ),
...
function coplayedCardView({ authorData }) {
    ...
    return ce(Pie, {
        ...
        urlformatter: evt =>
            browserHistory.push(
                buildURL(`/author/${evt.point.object.author_id}`),
            ),
    });
}
...
class SeasonCardView extends Component {
    render() {
        return ce(HighchartsComparator, {
            urlformatter: evt =>
                buildURL(
                    `/aseason/${this.props.authorData.id}/${evt.point.name}`,
                ),
            seriedef: {
        ...
        if (!this.props.loading) {
            return ce(
                ...
                ce(
                    'section',
                    { className: 'section--center mdl-grid' },
                    ce(authorGenreStatView, {
                        authorData: this.props.mainentity,
                    }),
                    ce(authorPlaysOverview, {
                        authorData: this.props.mainentity,
                    }),
                ),
                ce(
