...
function buildCalendar(data, domselector) {
    ...
    data.forEach(d => {registersByDay[dateFormatter(new Date(d.date))] = d});
    ...
    const firstDay = new Date(firstDate.getFullYear(),
                              firstDate.getMonth(),
                              1),
    ...
    const svg = d3.select(domselector).selectAll('svg')
    ...
    if (tooltip === null) {
        tooltip = d3.select('body')
            .append('div').attr('id', 'scalendar-tooltip')
    ...
    svg.selectAll('.calendar-daylabel')
        .data(weekdays)
        .enter().append('text')
        .text(d => d)
    ...
    svg.selectAll('.calendar-monthlabel')
        .data(d3.time.months(firstDate, lastDate, 3))
        .enter().append('text')
        .text(d => monthnames[d.getMonth()])
        .attr('x', d => leftmargin + cellsize * (Math.floor(daysBetween(d, firstDay) / 7)))
    ...
    const rect = grect.selectAll('.calendar-day')
          .data(plottedData)
          .enter().append('rect')
          .attr('class', dayclass)
          .attr('width', cellsize)
          .attr('height', cellsize)
          .style('fill', d => (d.free || !d.inrange || !d.register)
                 ? '#fff'
                 : color(d.receipts))
...
export class SeasonCalendar extends Component {
    ...
    render() {
        return ce('div', {style: {width: '80%', margin: '0 auto', textAlign: 'center'}},
