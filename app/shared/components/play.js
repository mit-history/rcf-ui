    ...
    render() {
        ...
        return ce(Card, {className: mdlclass(12), style: {fontSize: "1em"}},
                  ce(CardText, {style: {margin: "auto",  fontSize: "1em"}},
                     ce(BootstrapTable, {options: options, data: plays,
                                         hover: true, pagination: true},
...
function playMainCardView({playData}) {
    ...
    const pictoDescrLabelStyle = {fontSize: "14px", display: "block", color: "rgba(0,0,0,.54)", borderBottom: "1px solid #009687"};
    const pictoLabelStyle = {fontSize: "18px", color: "rgba(0,0,0,.54)", marginTop: "15px"}

    const links = [{
        img: 'https://cdn4.iconfinder.com/data/icons/infy-basic-collection/53/user-128.png',
        label: 'Auteur',
        value: playData.author,
        url: `/author/${playData.author_id}`,
    }, {
        img: 'https://cdn2.iconfinder.com/data/icons/chat-and-sms/64/sms_chat_web_text-128.png',
        label: 'Discours',
        value: playData.prose_vers,
    }, {
        img: 'https://cdn0.iconfinder.com/data/icons/constructivism-for-the-bank/64/constr_account_statements-128.png',
        label: 'Actes',
        value: playData.acts,
    }, {
        img: 'https://cdn2.iconfinder.com/data/icons/ballicons-2-free/100/theatre-128.png',
        label: 'Genre',
        value: playData.genre,
        url: `/genre/${playData.genre}`,
    }, {
        img: 'https://cdn2.iconfinder.com/data/icons/rafif-rounded-flat-vol-3/512/music-128.png',
        label: 'Musique / Danse / Machie',
        value: playData.musique_danse_machine === false ? 'non' : 'oui',
    }, {
        img: 'https://cdn1.iconfinder.com/data/icons/vote-and-rewards-1/65/49-128.png',
        label: 'Prologue',
        value: playData.prolog === false ? "non" : "oui",
    }];

    return ce('section', {className: 'section--center mdl-shadow--2dp', style: {justifyContent: "center"}},
              ce('section', {className: 'section--center', style: {marginBottom: "50px"}},
                 ce('h2', {style: {textAlign: "center"}},
                    ce('img', {src:"https://cdn2.iconfinder.com/data/icons/game-assets-3/1000/RedRibbon.png",
                               style:{width:"250px", display:"block", margin: "auto"}}),
                    playData.title)
                ),
              ce('section', {className: 'section--center mdl-grid', style: {justifyContent: "center"}},
                 ...links.map(linkdef => {
                     let value = ce('span', {style: pictoLabelStyle}, linkdef.value);
                     if (linkdef.url) {
                         value = ce(Link, {to: linkdef.url}, value);
                     }
                     return ce('div', {className:playOverviewClassNames,
                                       style: playOverviewStyle},
                               ce('img', {src: linkdef.img, style: {width:"60px"}}),
                               ce('p', {style: {padding: "10px 60px 10px 60px"}},
                                  ce('span', {style: pictoDescrLabelStyle}, linkdef.label),
                                  value));
                 })));
}
...
class PerformanceCardView extends Component {
    ...
    render() {
        ...
        return ce(Card, {className: mdlclass(12)},
                  ce(CardTitle, {style: STYLES.titleChart},`Performances (${nbPerfs})`),
    ...
    render() {
        if (this.props.loading === false) {
            return ce('section', null,
                      ce('div', {id: 'overview'}, ce(playMainCardView, {playData: this.props.mainentity})),
...
export function repriseList({reprises, loading}) {
    if (loading === false) {
        ...
        return ce(Card, {className: mdlclass(12), style: {fontSize: "1em"}},
                  ce(CardText, {style: {margin: "auto",  fontSize: "1em"}},
                     ce(BootstrapTable, {options: options, data: reprises,
                                         hover: true, pagination: true},
                        ce(TableHeaderColumn, {dataField: "date",
                                               isKey: true, hidden: true}, 'ID'),
